//! Production script-pool sites audit (historical name `rayon_audit`).
//!
//!   cargo bench -p rbitcoin-consensus --bench rayon_audit
...
fn bench(name: &str, iters: u32, mut f: impl FnMut()) {
    ...
    println!("{name:56}  {:>10.2?}/op", t0.elapsed() / iters.max(1));
}
...
fn main() {
    ...
    println!("script_pool_audit (bench name: rayon_audit)  cpus={cpus:?}\n");
    for (label, n, iters) in [
        ("1 job", 1u32, 100u32),
        ("8 jobs", 8, 40),
        ("64 jobs", 64, 15),
    ] {
        let jobs: Vec<_> = (0..n).map(|i| p2wpkh_job((i % 40 + 1) as u8)).collect();
