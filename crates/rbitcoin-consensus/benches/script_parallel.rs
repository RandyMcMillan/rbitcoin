//! Sequential vs script_pool batch sizes (post-rayon).
//!
...
fn main() {
    ...
    println!("script_parallel — pool vs sequential  cpus={cpus:?}\n");
    for n in [1usize, 8, 64, 256] {
        let jobs: Vec<_> = (0..n).map(|i| p2wpkh_job((i % 100 + 1) as u8)).collect();
        let owned = script_bench::owned_jobs(&jobs);
        println!("--- n={n} ---");
        let iters = if n >= 256 { 5 } else { 15 };
