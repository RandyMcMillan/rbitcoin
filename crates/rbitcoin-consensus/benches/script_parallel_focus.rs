...
fn main() {
    ...
    for n in [2usize, 3, 4, 8, 16, 32, 64, 128] {
        let jobs = script_bench::owned_jobs(
            &(0..n as u8)
                .map(|i| p2tr(i.wrapping_add(5)))
                .collect::<Vec<_>>(),
        );
        let iters = match n {
            0..=4 => 500u32,
        ...
        let fine = measure(iters, || {
            jobs.par_iter()
                .try_for_each(|j| script_bench::verify_one_job(j))
                .unwrap();
        });
        let min4 = measure(iters, || {
            jobs.par_iter()
                .with_min_len((n / (t * 4)).max(1).min(8))
                .try_for_each(|j| script_bench::verify_one_job(j))
                .unwrap();
        });
