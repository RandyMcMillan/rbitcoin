...
impl Query {
    ...
    pub fn load_thin_tweaks(
        ...
        let i0 = u64::from(elig[0].0);
        let i1 = u64::from(elig[elig.len() - 1].0);
        let txids = self
            .store
            .txs
            .body_txid_range(first_id + i0, first_id + i1)?;
        let ranges = self.store.txs.body_ranges(first_id + i0, first_id + i1)?;
        if txids.len() != ranges.len() || ranges.is_empty() {
            return Err(StoreError::Corrupt("invariant: thin span idx"));
        }
        let span_lo = ranges[0].0;
        let (last_off, last_len) = ranges[ranges.len() - 1];
        let span_hi = last_off.saturating_add(last_len);
        if span_hi < span_lo {
        ...
        self.store
            ...
            .with_body_span(span_lo, span_hi - span_lo, |blob| {
                ...
                for &(tx_i, tweak) in &elig {
                    let rel_i = (u64::from(tx_i) - i0) as usize;
                    let (off, len) = ranges[rel_i];
                    ...
                    let p2tr = self.store.txs.packed_p2tr_from_raw(&blob[rel..end])?;
