//! Streaming archive head-resolve: mmap probe + mmap idx + io_uring body prefixes.
//!
//! Completion-driven loop (one outstanding body pread per in-flight key):
//! 1. mmap `probe_fks` → candidates deepest-first (BIP30)
//! 2. mmap `record_range` for next cand
//! 3. io_uring pread ≤33 body bytes; on match → done for key; else next cand
//!
...
pub fn resolve_batch_streaming(
    ...
    while in_flight_keys > 0 {
        ...
        for (ud, res) in cqes {
            ...
            if res as usize != work.buf_len {
                free_slots.push(slot);
                return Err(StoreError::Corrupt("stream body pread short"));
            }

            let key_i = work.key_i as usize;
            let want = &txids[key_i];
            let matched = match TxTable::txid_from_body_prefix(&work.buf[..work.buf_len]) {
            ...
            if matched {
                results[key_i] = Some(Fk(work.pending_fk));
                done[key_i] = true;
                free_slots.push(slot);
            } else {
                // Need to re-submit with buffer in `slots[slot]` so the pointer
                // stays stable for the duration of the in-flight pread.
                slots[slot] = Some(work);
                let submitted = {
                    let w = slots[slot].as_mut().unwrap();
                    try_submit_body(table, w, &mut session, body_fd, body_pub, slot as u32, &mut idx_ns)?
                };
                if submitted {
                    in_flight_keys += 1;
                } else {
...
fn arm_keys(
        ...
        && !free_slots.is_empty()
    {
        ...
        *cands_total = cands_total.saturating_add(raw.len() as u64);
        ...
        // Place work in slot first so the body buffer address is stable.
        ...
        let submitted = {
            let w = slots[slot].as_mut().unwrap();
            try_submit_body(table, w, session, body_fd, body_pub, slot as u32, idx_ns)?
        };
        if submitted {
            *in_flight_keys += 1;
        } else {
...
fn try_submit_body(
    ...
    while work.cand_i < work.cands.len() {
        ...
        let range = match table.body.record_range(Fk(fk)) {
            Ok(r) => r,
            Err(StoreError::InvalidFk) | Err(StoreError::NotFound) => {
        ...
        *idx_ns = idx_ns.saturating_add(t_idx.elapsed().as_nanos() as u64);
        ...
        work.buf_len = n;
        work.pending_fk = fk;
        session.push_pread(body_fd, off, &mut work.buf[..n], u64::from(slot))?;
        return Ok(true);
    }
    Ok(false)
}
