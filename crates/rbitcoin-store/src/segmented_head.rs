...
/// Read-only segment view for plan head-resolve (open → sealed newest→oldest).
pub(crate) struct ResolveSeg {
    pub first_fk: u64,
    /// Index in the full segment list (for DONTCACHE sealed-age).
    pub si: usize,
    pub n_segs: usize,
    pub head: Arc<AddressHead>,
    /// Sealed fuse only; open has `None`.
    pub fuse: Option<SealedFuse8>,
}

impl ResolveSeg {
    #[inline]
    pub fn fuse_contains(&self, mixed: &[u8; 32]) -> bool {
        match &self.fuse {
            None => true, // open: always probe
            Some(f) => f.contains(fuse_key_from_mixed(mixed)),
        }
    }

    #[inline]
    pub fn dontcache(&self) -> bool {
        crate::dontcache_policy::head_or_idx_segment_index(self.si, self.n_segs)
    }

    #[inline]
    pub fn rel_to_abs(&self, rel: u64) -> Option<Fk> {
        rel_to_abs(self.first_fk, rel)
    }
}

impl SegmentedTxHead {
    /// Segments in **resolve order**: open first (if any), then sealed newest→oldest.
    pub(crate) fn resolve_segments(&self) -> Vec<ResolveSeg> {
        let segs = self.segments_snapshot();
        let n_segs = segs.len();
        if n_segs == 0 {
            return Vec::new();
        ...
        if !last.sealed {
            out.push(ResolveSeg {
                first_fk: last.first_fk,
                si: n_segs - 1,
                n_segs,
                head: Arc::clone(&last.head),
                fuse: None,
            });
        }
        let sealed_range: Box<dyn Iterator<Item = usize>> = if last.sealed {
