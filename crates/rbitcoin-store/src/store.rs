...
impl Store {
    ...
    /// Structural write IO wave: Class A body 9B spender meta **and** `tx_height`
    /// 4B slots in **one** [`crate::bulk_io`] submit (multi-fd).
    ///
    /// Meta and heights are independent once pin abs offsets and create fks are
    /// known — serial meta-then-height doubled disk wait. Empty sides fall back
    /// to the dedicated bulk helpers. Backend: uring if either
    /// `RBITCOIN_SPEND_META` or `RBITCOIN_CLASS_C_IO` (or global `RBITCOIN_IO`)
    /// selects uring.
    pub fn structural_meta_and_tx_height_batch(
        &self,
        abs_offs: &[u64],
        height_fks: &[Fk],
    ) -> Result<(Vec<Option<(Fk, u8)>>, Vec<Option<u32>>), StoreError> {
        use crate::bulk_io::{self, ReadOp};
        use crate::io_backend::ReadIoBackend;

        if abs_offs.is_empty() && height_fks.is_empty() {
            return Ok((Vec::new(), Vec::new()));
        }
        if height_fks.is_empty() {
            return Ok((self.get_spender_meta_at_abs_batch(abs_offs)?, Vec::new()));
        }
        if abs_offs.is_empty() {
            return Ok((Vec::new(), self.tx_height_get_batch(height_fks)?));
        }

        let backend = match (
            crate::io_backend::spend_meta_io_backend(),
            crate::io_backend::class_c_io_backend(),
        ) {
            (ReadIoBackend::Uring, _) | (_, ReadIoBackend::Uring) => ReadIoBackend::Uring,
            _ => ReadIoBackend::Pread,
        };

        const META_LEN: usize = 9;
        let body_fd = self.txs.body.body_read_fd();
        let body_pub = self.txs.body.body_published_len();
        let body_path = self.txs.body.body_file_path();
        let height_fd = self.tx_height.read_fd();
        let nslots = self.tx_height.slot_count();

        let mut meta_bufs: Vec<[u8; META_LEN]> = vec![[0u8; META_LEN]; abs_offs.len()];
        let mut meta_submitted: Vec<usize> = Vec::with_capacity(abs_offs.len());
        for (i, &off) in abs_offs.iter().enumerate() {
            let end = off.saturating_add(META_LEN as u64);
            if end > body_pub {
                continue;
            }
            meta_submitted.push(i);
        }

        let mut height_out: Vec<Option<u32>> = vec![None; height_fks.len()];
        let mut height_bufs: Vec<[u8; 4]> = vec![[0u8; 4]; height_fks.len()];
        let mut height_submitted: Vec<(usize, u64)> = Vec::with_capacity(height_fks.len());
        for (i, fk) in height_fks.iter().enumerate() {
            ...
            height_submitted.push((i, index));
        }

        // SAFETY: each bufs[i] is distinct; submitted indices unique.
        let mut ops: Vec<ReadOp<'_>> =
            Vec::with_capacity(meta_submitted.len() + height_submitted.len());
        for &i in &meta_submitted {
            let ptr = meta_bufs[i].as_mut_ptr();
        ...
        let meta_op_count = ops.len();
        for &(i, index) in &height_submitted {
            let ptr = height_bufs[i].as_mut_ptr();
            let slice = unsafe { std::slice::from_raw_parts_mut(ptr, 4) };
            ops.push(ReadOp {
                fd: height_fd,
                offset: crate::chain::TxHeightTable::offset_for_index(index),
                buf: slice,
                result: i32::MIN,
                dontcache: false,
            });
        ...
        let mut metas: Vec<Option<(Fk, u8)>> = vec![None; abs_offs.len()];
        for (ro, &i) in ops[..meta_op_count]
            .iter()
            .zip(meta_submitted.iter())
        {
            if ro.result < 0 {
            ...
            let flags = b[8];
            metas[i] = Some((field, flags));
        }
        for (ro, &(i, _)) in ops[meta_op_count..]
            .iter()
            .zip(height_submitted.iter())
        {
            if ro.result != 4 {
                continue;
            }
            let v = u32::from_le_bytes(height_bufs[i]);
            if v != 0 {
                height_out[i] = Some(v - 1);
            }
        }
        Ok((metas, height_out))
    }
...
mod tests {
    ...
    /// Combined structural wave matches separate meta + height bulk reads.
    #[test]
    fn structural_meta_and_tx_height_wave_matches_separate() {
        let dir = tmp();
        let s = Store::create(&dir).unwrap();
        let create = (
            TxRecord {
        ...
        let create_fk = s.put_tx_full_batch_indexed(&[create], true).unwrap()[0];
        s.tx_height.set(create_fk, Height(7)).unwrap();
        let (off, len) = s.tx_body_range(create_fk).unwrap();
        let decoded = s
            .get_tx_meta_and_outputs_batch_at(&[(off, len)])
            .unwrap();
        let dens = decoded[0].as_ref().unwrap().2[0];
        let abs = off + u64::from(dens);

        let separate_meta = s.get_spender_meta_at_abs_batch(&[abs]).unwrap();
        let separate_h = s.tx_height_get_batch(&[create_fk]).unwrap();
        let (wave_meta, wave_h) = s
            .structural_meta_and_tx_height_batch(&[abs], &[create_fk])
            .unwrap();
        assert_eq!(wave_meta, separate_meta);
        assert_eq!(wave_h, separate_h);
        assert_eq!(wave_h[0], Some(7));
        // Null field unspent on fresh create.
        let (field, _flags) = wave_meta[0].unwrap();
        assert!(field.is_null());

        // Empty sides.
        assert!(s
            .structural_meta_and_tx_height_batch(&[], &[])
            .unwrap()
            .0
            .is_empty());
        let only_h = s
            .structural_meta_and_tx_height_batch(&[], &[create_fk])
            .unwrap();
        assert!(only_h.0.is_empty());
        assert_eq!(only_h.1, separate_h);
