...
**Height-ordered unified pipeline (current):** peer → **body queue** → **lookup** (BQ-ahead TipOnly `head_fk` wave) → **load** (structure + stamp from BQ hits + leftover load-owned-pending-then-TipOnly + pin `txout` + assemble) → scripts → single commit era. **No** peer→confirm-feed wire retain. **No** hash-only / Class-A-only confirm (bq wire required). Load leftover order is in-flight → pins → BQ hits → **load-owned pending (no fence)** → TipOnly (fence-connected). Write drain inserts `tx.head` in parallel with Class C; leftover identity is a **plain map on the load thread** (write inbox `Note` / `DrainDone` / `TipAdvanced` / `LayoutDone`). Forget is load-only after DrainDone + fence + height `< pack_lo`. In-flight prune is **leftover-ready** (`covers_fk_span` of the pack's fks). Bodies without a known height are marked missing and re-getdata after the height map is ready — there is **no** dual-track archive-job / ContigPark fallback.
