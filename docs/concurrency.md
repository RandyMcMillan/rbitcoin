...
**Height-ordered unified pipeline (current):** peer → **body queue** → **lookup** (BQ-ahead TipOnly `head_fk` wave) → **load** (structure + stamp from BQ hits + in-flight + TipOnly `tx.head` + pin `txout` + assemble) → scripts → single commit era. **No** peer→confirm-feed wire retain. **No** hash-only / Class-A-only confirm (bq wire required). Load bind order is in-flight → pins → BQ hits → TipOnly (fence-connected). Write drain inserts `tx.head` in parallel with Class C. Drain complete is max inserted **fk** (not tip/fence — those advance during drain). Header-cache GC polls store tip every load pack. In-flight prune is **after bind**: drain-fk **and** `covers_fk_span` (TipOnly home — `docs/invariants.md`). No leftover pending map. Bodies without a known height are marked missing and re-getdata after the height map is ready — there is **no** dual-track archive-job / ContigPark fallback.
