...
**`ibd: perf`:** `load=` is pin+assemble only. Load OS-thread leftover TipOnly is `load_thr stamp=`; post-scriptq in-flight drop is `prune=`. `script=` is verify / `wait_done` wall (`jobs=`/`skip=`), not the feed-ahead join poller. Steal claim is an `ArcSwap` snapshot + `fetch_add(32)` (no `WAVES` mutex per job; `in_wave` counts in-flight chunks). After N+1 is on the second coordinator, scripts join **blocks** (200 µs `recv_timeout` only while lookahead is still empty). `ready>0` + `scriptq=1` + high `stamp=` means leftover on load, not a hungry script pool.
