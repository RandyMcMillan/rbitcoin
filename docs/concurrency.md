...
**Height-ordered unified pipeline (current):** peer → **body queue** → **lookup** (BQ-ahead TipOnly `head_fk` wave) → **load** (structure + stamp from BQ hits + leftover load-owned-pending-then-TipOnly + pin `txout` + assemble) → scripts → single commit era. **No** peer→confirm-feed wire retain. **No** hash-only / Class-A-only confirm (bq wire required). Load leftover order is in-flight → pins → BQ hits → **load-owned pending (no fence)** → TipOnly (fence-connected). Write drain inserts `tx.head` in parallel with Class C; leftover identity is a **plain `txid → fk` map on the load thread**. Inbox is notes only. Drain complete is max inserted **fk** (not tip/fence — those advance during drain). Header-cache GC polls store tip every load pack. Forget is per-fk after bind. In-flight prune is **layer** drain-fk **and** `covers_fk_span` (TipOnly home — `docs/invariants.md`). Bodies without a known height are marked missing and re-getdata after the height map is ready — there is **no** dual-track archive-job / ContigPark fallback.
