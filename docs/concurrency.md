...
| Confirm **plan** | 1 OS thread | load wire from **body queue**; structure + **plan** Class A (stamp create_fk) + ensure external denserels into CreateResidency |
| Confirm **prep** | 1 OS thread | pin parents (Forbid — denserels already warm) + assemble; **no** re-plan |
...
**Height-ordered unified pipeline (current):** peer → **body queue** → **plan** (structure + stamp create_fk + ensure external denserels) → **prep** (pin Forbid + assemble) → scripts → single commit era. **No** peer→confirm-feed wire retain. **No** hash-only / Class-A-only confirm (bq wire required). Prep must **not** re-run plan head resolve. Optional **archive-job + ContigPark** remains only for unknown-height bodies.
