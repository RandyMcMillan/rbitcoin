...
- Scripthash (Direct): thin creates → memtable → target-sized sorted spills + **SEAL** (`max_create_fk`). Memtable is not durable; on resume, re-enqueue creates with `create_fk > SEAL`. Tip: **direct k-way** stream of claimed runs into live OA shards (fan-in reduce only if catalog ≫ max direct). **SIGINT mid cold stream:** complete prefix shards stay on disk; `scripthash.cold_progress` records `next_shard` + body HWM; restart **skips** earlier stream bands and continues. Mid-reduce keeps `merge/CHECKPOINT`. Deferred catch-up runs after cold load use **batched warm apply** (10s status). **Legacy 16-way `scripthash.head/`:** if runs exist, open migrates to empty 64-way + tip rebuild. No runs and wrong shard count ⇒ reindex.
