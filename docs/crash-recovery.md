...
- Scripthash (Direct): thin creates → memtable → target-sized sorted spills + **SEAL** (`max_create_fk`). Memtable is not durable; on resume, re-enqueue creates with `create_fk > SEAL`. Tip: **one** dynamic fan-in pass (per-chunk `merge/CHECKPOINT`, inputs deleted as chunks finish) then cold bulk-load; **SIGINT** resumes CHECKPOINT/READY. New catalog runs after interrupt are deferred and warm-inserted after materialize. Corrupt durable SH ⇒ reindex.
