...
- Scripthash (Direct): thin creates → memtable → target-sized sorted spills + **SEAL** (`max_create_fk`). Memtable is not durable; resume re-enqueues Class A with `create_fk > SEAL`. Tip path:
  - **Full cold** when head empty (after catalog is complete) or **`RBITCOIN_SH_FORCE_REBUILD=1`** (wipes head+runs+SEAL+HWM, streams Class A→runs with status/SEAL resume, then cold load; empty catalog after recollect is fatal).
