...
- Scripthash (Direct): thin creates → memtable → target-sized sorted spills + **SEAL** (`max_create_fk`). Memtable is not durable; on resume, re-enqueue creates with `create_fk > SEAL`. Tip: **direct k-way** stream of claimed runs into live OA shards (fan-in reduce only if catalog ≫ max direct); **SIGINT** mid-stream reinit+re-stream; mid-reduce keeps `merge/CHECKPOINT`. New catalog runs after interrupt are deferred and warm-inserted after materialize. **Legacy 16-way `scripthash.head/`:** if `scripthash.runs` (catalog / `.run.mat` / merge READY|CHECKPOINT) exists, open **moves** the old head aside (`scripthash.head.legacy-*`), creates empty 64-way head, clears SH body, and tip entry rebuilds from runs. No runs and wrong shard count ⇒ reindex.
