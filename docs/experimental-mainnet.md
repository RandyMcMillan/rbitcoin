...
| Confirm lookup/load | Lookup packs by soft **Σ inputs** (hardcoded **8000**) or hard **144** blocks — at dense mainnet heights that is typically **a few blocks per batch** (often 1–3; live `n=`), not ~32-block waves. Queue depths hardcoded loadq=8 · scriptq=4 · writeq=20 |
