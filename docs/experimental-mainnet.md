...
| Confirm plan/prep | Plan packs by soft **Σ inputs** (default **8000**, `RBITCOIN_CONFIRM_BATCH_INPUTS`) or hard **144** blocks — at dense mainnet heights that is typically **a few blocks per plan** (often 1–3; live `n=`), not ~32-block waves. Queue defaults `planq=*/1 prepq=*/1 writeq=*/10` (env may raise writeq) |
