...
| Confirm lookup/load | **Load** packs by soft **Σ inputs** (hardcoded **8000**) or hard **144** blocks — dense mainnet is typically **a few blocks per batch** (often 1–3; live `n=`), not ~32-block waves (8000/250 is mid-chain, not fat-era). IBD **lookup** TipOnly-resolves at most **8** BQ-ready heights per wave. Queue depths hardcoded loadq=8 · scriptq=4 · writeq=20 |
