...
| Confirm load | Dense heights are pin-bound; expect `conf blks=32` with `loadq` low relative to cap (`loadq=*/5`). If tip ~1 blk/s and scripts idle, check for `confirm load multi-block fail` (should be rare after BIP68 plan-MTP fix) |
