...
| **load** | Stamp from BQ hits + in-flight / pins, then leftover TipOnly (2-wave; open + ages ≤3). Pins `txout` by stamped range. In-flight holds **unconfirmed** planned creates and is pruned when `tip >= pack max_height`. Leftover is **confirmed / fence-connected** parents only. |
| **scripts** | No store. |
| **write** | Sole Class A appender; `head_insert_many` write-behind. Drain may finish before `height_fence_extend` — unconfirmed packs stay in-flight until tip covers them. |
