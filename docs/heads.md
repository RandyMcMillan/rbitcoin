...
| **scripts** | No store. |
| **write** | Sole Class A appender; `head_insert_many` write-behind. Drain ∥ Class C. Write queued is insert-only. Leftover home is the load inbox map until DrainDone + fence + height `< pack_lo`. Write never forgets leftover identity (`67438` / n−1). RPC `get_fk_by_txid` hits durable head only until drain. |
