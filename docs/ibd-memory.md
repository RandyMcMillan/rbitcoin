...
| **In-flight CreatePin layers** | identity + full create outs; drop when drain+fence height > `until` stamped at `note_layer` (`until = lookup_started_hi`, or pack `max_height` if `None`). Horizon is `drain_and_fence_hi`, not Class C tip | Load notes at stamp; disconnect `drop_from` on pack height. Sizes: `iflight=`. Not a coins cache / spend FIFO |
