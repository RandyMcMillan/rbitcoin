...
| **GetData serve inflight** | **16** full `Block`/`CmpctBlock` per session writer | Writer saturating-decrements after send. Compact tip announce is queued on the same counter so an unpaired sub cannot wrap to `usize::MAX`. Extra inv hashes not reconstructed. |
