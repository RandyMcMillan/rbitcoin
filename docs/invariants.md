...
| Leftover parent union | A committed create is leftover-visible via **in-flight** until `fence.covers_fk_span`, **load-owned pending** (write inbox `Note`, no fence) until load sees `DrainDone` **and** fence cover **and** height `< pack_lo`, then **TipOnly head**. Write queued is insert-only. Forget is load-only (keep previous pack). Disconnect drops that height immediately. RAM maps are one fk per txid — [`errata.md`](./errata.md). | **No** soft-requeue. Union miss → `Corrupt("parent create_fk unresolved")` (permanent) |
