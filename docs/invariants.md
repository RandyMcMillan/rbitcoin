...
| Parent create_fk union | **in-flight** (prune **after** pin + scripts handoff: drain inserted fk span **and** `fence.covers_fk_span`) → **published live_union** (height-layered chain, `ArcSwap` of the head) → **TipOnly** (connected **and** idx `body_range`). No leftover pending map. Lookup drops a layer once that height has left the BQ. Disconnect `store(None)` immediately and clears the lookup union. Header-cache GC polls store tip each load pack. One fk per txid — [`errata.md`](./errata.md). | **No** soft-requeue. Union miss → `Corrupt("parent create_fk unresolved")` (permanent). Identity without idx range → `Corrupt("invariant: idx range missing after identity")`, not a miss |
