...
| Leftover parent union | A committed create is leftover-visible via **in-flight** until `fence.covers_fk_span`, **pending snap** until `head_insert_many` has published *and* `height_of` is Some, then **TipOnly head**. Forget only after `drain.join()` (insert done) *and* fence extend — not from Class C while drain is in flight. Still-queued keys stay in the snap. | **No** soft-requeue. Union miss → `Corrupt("parent create_fk unresolved")` (permanent) |
