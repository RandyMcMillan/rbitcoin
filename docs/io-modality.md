...
| SH Electrum/Esplora join | `RBITCOIN_IO` | Query-thread session: waved `idx_body_pipeline` on **`txout.body`**, page-grouped **`txid.body`**, `get_spender_meta_at_abs_batch` on **`spent.body`**. Megakey page chain: one span pread when pages are contiguous at bump; else linked 4 KiB walk. Does **not** share a confirm TLS ring. |
