...
**Last reaudit:** 2026-08-17 (docs map #81, Core functional **34** `run`,
...
| **Q-41** | Keep. 9 → **34** `run` (Waves A–D leftovers + leftover-P2P `p2p_initial_headers_sync`). 233 skips remain; `rpc-missing` + `core-log` are the only growth that matches claimed surface. |
...
| **—** | Core functional `run` set | **34** unmodified v31.1 scripts (was 9). Remaining growth is **Q-41** |
