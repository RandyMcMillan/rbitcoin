...
| RB-009 | Structure / BIP143 midstates | `compute_txid`/`compute_wtxid`/`block.weight` plus `SighashCache` rewalk prevouts | `TxPrecompute` one-pass (Core `PrecomputedTransactionData` shape); rust-bitcoin is the unit oracle | `block/tx_precompute.rs`; `script::crypto::bip143_*` | mitigated | SHA-NI already in `bitcoin_hashes` 0.14; do not add `sha2` for that |
