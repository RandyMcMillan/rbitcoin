...
use rbitcoin_fuzz::{
    check_diff_env, compare_fork_one, diff_regtest_params, mine_diff_pad, mine_diff_stem,
...
fn harness_failure(what: &str) -> ! {
    eprintln!("=== BLOCK-FORK-DIFFERENTIAL FUZZ HARNESS FAILURE ===");
    eprintln!("{what}");
...
fn note_comparison() {
    ...
    if n == 1 || n.is_multiple_of(100) {
        eprintln!("block-fork-differential: comparisons={n}");
    }
}
...
fn base() -> &'static Base {
    BASE.get_or_init(|| {
        ...
        let store = tmp_dir("rbtc-fork-diff-store");
        let core_dir = tmp_dir("rbtc-fork-diff-core");
