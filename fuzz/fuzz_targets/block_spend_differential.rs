...
use rbitcoin_fuzz::{
    check_diff_env, compare_spend_one, diff_regtest_params, mine_diff_pad, submit_pad_to_oracle,
...
fn harness_failure(what: &str) -> ! {
    eprintln!("=== BLOCK-SPEND-DIFFERENTIAL FUZZ HARNESS FAILURE ===");
    eprintln!("{what}");
...
fn note_comparison() {
    ...
    if n == 1 || n.is_multiple_of(100) {
        eprintln!("block-spend-differential: comparisons={n}");
    }
}
...
fn base() -> &'static Base {
    BASE.get_or_init(|| {
        ...
        let store = tmp_dir("rbtc-spend-diff-store");
        let core_dir = tmp_dir("rbtc-spend-diff-core");
        ...
        Base { hub, core, tip: std::sync::Mutex::new(pad.tip.clone()), pad, _store: store }
    })
}
...
fuzz_target!(|data: &[u8]| {
    ...
    match compare_spend_one(&b.hub, &mut tip, &b.core.rpc, b.pad.mature, data) {
        CompareOne::NotABlock | CompareOne::Skipped => {
            ORACLE_DOWN_STREAK.store(0, Ordering::Relaxed);
        }
        CompareOne::Agreed { .. } => note_comparison(),
        CompareOne::Disagreed { ours, core, hex } => {
            eprintln!("=== BLOCK-SPEND-DIFFERENTIAL FUZZ CONSENSUS DIVERGENCE ===");
            eprintln!("ours_accept={ours} core_accept={core}");
            eprintln!("block_hex={hex}");
            panic!("block-spend-differential: ours={ours} core={core} hex={hex}");
        }
