...
use rbitcoin_fuzz::{
    check_diff_env, compare_cmpct_reorg_one, diff_regtest_params, mine_diff_pad, mine_diff_stem,
...
fn harness_failure(what: &str) -> ! {
    eprintln!("=== CMPCT-REORG-DIFFERENTIAL FUZZ HARNESS FAILURE ===");
    eprintln!("{what}");
...
fn note_comparison() {
    ...
    if n == 1 || n.is_multiple_of(100) {
        eprintln!("cmpct-reorg-differential: comparisons={n}");
    }
}
...
fn base() -> &'static Base {
    BASE.get_or_init(|| {
        ...
        let store = tmp_dir("rbtc-cmpct-reorg-diff-store");
        let core_dir = tmp_dir("rbtc-cmpct-reorg-diff-core");
...
fuzz_target!(|data: &[u8]| {
    let b = base();
    match compare_cmpct_reorg_one(&b.hub, &b.pad, &b.core.rpc, data) {
        ...
        CompareOne::Disagreed { ours, core, hex } => {
            ...
            panic!("cmpct-reorg-differential: ours={ours} core={core} hex={hex}");
        }
