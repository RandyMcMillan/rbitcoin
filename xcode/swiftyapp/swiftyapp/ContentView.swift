                    ...
                    glassCard {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            Button {
                                ...
                                let utxoOverhead = rpcPerUtxoOverhead()
                                moreConstantsResult = "bw=\(bw) spb=\(spb) pkg=\(mpc)/\(mpw) rbfr=\(rnum)/\(rden) msgs=\(maxMsgs) bytes=\(maxBytes) rl=\(rlBan) os=\(osBan) addr=\(maxAddr)/\(maxPct) transit=\(transit) ibdWin=\(ibdWin) utxoOverhead=\(utxoOverhead)"
                            } label: {
